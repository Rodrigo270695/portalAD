...
export default function Index({ circuits, zonals, filters }: Props) {
    const [search, setSearch] = useState(filters.search || '');
    const [perPage, setPerPage] = useState(filters.per_page?.toString() || '10');
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [selectedCircuit, setSelectedCircuit] = useState<Circuit | undefined>(undefined);
    ...
    useEffect(() => {
        const timeoutId = setTimeout(() => {
            if (search !== filters.search) {
                ...
                const currentPage = filters.page || circuits.current_page;
                router.get('/circuits', {
                    search,
                    page: currentPage,
                    per_page: perPage
                    ...
                    preserveState: true,
                    preserveScroll: true,
                    only: ['circuits', 'filters']
                });
            }
        }, 300);

        return () => clearTimeout(timeoutId);
    }, [search, filters.search, filters.page, circuits.current_page, perPage]);
    ...
    const handleDelete = (id: number, name: string) => {
        Swal.fire({
            title: '¿Estás seguro?',
            text: `¿Realmente deseas eliminar el circuito "${name}"?`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                router.delete(`/circuits/${id}`, {
                    onSuccess: () => {
                        Swal.fire({
                            title: 'Eliminado',
                            text: `El circuito "${name}" ha sido eliminado correctamente.`,
                            icon: 'success',
                            timer: 2000,
                    ...
                    onError: () => {
                        Swal.fire({
                            title: 'Error',
                            text: 'No se pudo eliminar el circuito. Intenta nuevamente.',
                            icon: 'error',
    ...
    const openEditModal = (circuit: Circuit, size: ModalSize = 'md') => {
                ...
                <div className="grid grid-cols-1 gap-4 md:hidden">
                    {circuits.data.length === 0 ? (
                        <div className="text-center py-6">
                            <p className="text-muted-foreground">No se encontraron circuitos</p>
                        </div>
                    ) : (
                ...
                {circuits.data.length > 0 && (
                    <div className="mt-4 flex flex-col sm:flex-row justify-between items-center gap-4">
                        <div className="text-sm text-muted-foreground">
                            Mostrando {circuits.from} a {circuits.to} de {circuits.total} registros
                        </div>
                        <Pagination links={circuits.links} className="mt-0" />
                    </div>
                )}
